...
class ReviewWorkspaceEndpointTest(unittest.TestCase):
    ...
    def test_workspace_phase2_provider_unlock_review_pack_is_preview_only(self):
        payload = {
            "workspace_id": "workspace-phase2-provider-unlock-review-preview",
            "source": "manual_import",
            "output_language": "en",
            "products": [{
                "platform": "manual",
                "asin": "PHASE2UNLOCK001",
                "title": "Adjustable Laptop Stand",
                "reviews": [
                    {
                        "rating": 2,
        ...
        self.assertEqual(response.status_code, 200)
        creative_pack = response.json()["creative_decision_pack"]
        self.assertIn("workspace_phase2_provider_unlock_review_pack", creative_pack)
        for existing_pack in [
        ...
        pack = creative_pack["workspace_phase2_provider_unlock_review_pack"]
        self.assertEqual(
            pack["pack_version"],
            "workspace_phase2_provider_unlock_review_pack_v1",
        )
        for required_key in [
        ...
        summary = pack["provider_unlock_review_summary"]
        self.assertIn("phase2_provider_unlock_review_preview", summary["mode"])
        self.assertIn("deterministic_unlock_governance", summary["mode"])
        self.assertIn("dry_run_only", summary["mode"])
        for source_pack in [
            "workspace_phase2_llm_provider_gate_pack",
            "workspace_phase2_persistence_mock_harness_pack",
        ...
        candidate_types = {
            ...
            for card in pack["provider_unlock_candidate_cards"]
        }
        for capability_type in [
            ...
            with self.subTest(capability_type=capability_type):
                self.assertIn(capability_type, candidate_types)
        for card in pack["provider_unlock_candidate_cards"]:
            for field in [
                "provider_unlock_candidate_id",
                "provider_label",
                "provider_group",
                ...
                with self.subTest(candidate=card["provider_unlock_candidate_id"], field=field):
                    self.assertIn(field, card)
            self.assertFalse(card["real_provider_call_allowed"])
        ...
        for card in pack["approval_requirement_review_cards"]:
            self.assertTrue(card["approval_token_required"])
            self.assertFalse(card["approval_token_issued"])
            self.assertFalse(card["real_task_creation_allowed"])
            self.assertFalse(card["token_issue_allowed"])
            self.assertFalse(card["real_execution_allowed"])

        secret_network_text = " ".join(
            " ".join(str(value) for value in card.values())
            for card in pack["secret_network_requirement_cards"]
        )
        for expected in ["API key", "secret storage", "rotation", "network allowlist"]:
            self.assertIn(expected, secret_network_text)
        for card in pack["secret_network_requirement_cards"]:
            self.assertFalse(card["secret_read_allowed"])
            self.assertFalse(card["external_call_allowed"])
            self.assertFalse(card["real_provider_call_allowed"])

        cost_quota_text = " ".join(
            " ".join(str(value) for value in card.values())
            for card in pack["cost_quota_review_cards"]
        )
        for expected in ["cost guard", "quota", "retry budget", "timeout"]:
            self.assertIn(expected, cost_quota_text)
        for card in pack["cost_quota_review_cards"]:
            self.assertTrue(card["cost_guard_required"])
            self.assertFalse(card["paid_operation_allowed"])
            self.assertFalse(card["real_provider_call_allowed"])

        sandbox_text = " ".join(
            " ".join(str(value) for value in card.values())
            for card in pack["sandbox_contract_test_review_cards"]
        )
        for expected in ["mock", "sandbox", "contract", "schema", "claim"]:
            self.assertIn(expected, sandbox_text)
        for card in pack["sandbox_contract_test_review_cards"]:
            self.assertTrue(card["schema_validation_required"])
            self.assertTrue(card["claim_safety_validation_required"])
            self.assertTrue(card["redaction_validation_required"])
            self.assertFalse(card["sandbox_test_executed"])
            self.assertFalse(card["real_provider_call_allowed"])

        for card in pack["audit_logging_requirement_cards"]:
            self.assertFalse(card["database_write_allowed"])
            self.assertFalse(card["real_audit_event_created"])
            self.assertFalse(card["real_log_read_allowed"])
        ...
        failure_text = " ".join(
            card["failure_type"]
            for card in pack["failure_recovery_review_cards"]
        )
        for failure_type in [
            "timeout",
            "rate limit",
        ...
        policy_text = " ".join(
            " ".join(str(value) for value in card.values())
            for card in pack["policy_claim_safety_dependency_cards"]
        )
        for expected in ["claim safety", "evidence grounding", "do_not_claim", "policy disabled"]:
            self.assertIn(expected, policy_text)
        for card in pack["policy_claim_safety_dependency_cards"]:
            self.assertFalse(card["real_policy_api_queried"])
            self.assertFalse(card["real_policy_check_allowed"])
            self.assertFalse(card["real_provider_call_allowed"])

        governance_text = " ".join(
            " ".join(str(value) for value in card.values())
            for card in pack["human_governance_review_cards"]
        )
        for expected in ["operator approval", "escalation", "manual review", "release owner", "rollback owner"]:
            self.assertIn(expected, governance_text)
        for card in pack["human_governance_review_cards"]:
            self.assertFalse(card["real_task_created"])
            self.assertFalse(card["real_approval_created"])
            self.assertFalse(card["real_execution_allowed"])

        for card in pack["provider_unlock_decision_cards"]:
            self.assertIn(card["deterministic_decision"], {"blocked", "not_ready", "preview_only"})
            self.assertNotEqual(card["deterministic_decision"], "ready_for_real_provider")
            self.assertFalse(card["ready_for_real_provider"])
            self.assertFalse(card["real_provider_call_allowed"])
            self.assertFalse(card["real_execution_allowed"])

        blockers = set(pack["phase2_provider_unlock_blockers"])
        for blocker in [
            "no provider key approval",
            "no secret access approval",
            "no external call approval",
        ...
        checks = pack["provider_unlock_quality_checks"]
        for key in [
            "candidate_providers_covered",
            "approval_requirement_covered",
            "secret_network_covered",
            "cost_quota_covered",
            "sandbox_tests_covered",
            "audit_logging_covered",
            "failure_recovery_covered",
            "policy_claim_safety_covered",
            "human_governance_covered",
            "decision_cards_covered",
            "unlock_blockers_covered",
            "safety_boundary_covered",
        ]:
            with self.subTest(check=key):
    ...
    def test_workspace_phase2_readiness_review_pack_is_preview_only(self):
        payload = {
            "workspace_id": "workspace-phase2-readiness-review-preview",
            "source": "manual_import",
            "output_language": "en",
            "products": [{
                "platform": "manual",
        ...
        self.assertEqual(response.status_code, 200)
        creative_pack = response.json()["creative_decision_pack"]
        self.assertIn("workspace_phase2_readiness_review_pack", creative_pack)
        for existing_pack in [
        ...
        pack = creative_pack["workspace_phase2_readiness_review_pack"]
        self.assertEqual(pack["pack_version"], "workspace_phase2_readiness_review_pack_v1")
        for required_key in [
        ...
        summary = pack["phase2_readiness_review_summary"]
        self.assertIn("phase2_readiness_review_preview", summary["mode"])
        self.assertIn("deterministic_unlock_gate_summary", summary["mode"])
        self.assertIn("dry_run_only", summary["mode"])
        for source_pack in [
            "workspace_phase2_provider_unlock_review_pack",
            "workspace_phase2_llm_provider_gate_pack",
            ...
            self.assertIn(source_pack, summary["source_packs"])
        for disabled_key in [
            "real_database_persistence_allowed",
            "real_llm_generation_allowed",
            "real_provider_call_allowed",
            "secret_read_allowed",
            "external_call_allowed",
            "token_issue_allowed",
        ...
        gate_ids = {card["gate_status_id"] for card in pack["phase2_gate_status_cards"]}
        for gate_id in [
            "database_persistence_gate",
            "persistence_mock_harness",
            "llm_provider_gate",
            "provider_unlock_review",
            "secret_environment_gate",
            "network_external_call_guard",
            "approval_token_gate",
            "cost_quota_guard",
            "audit_packet_preview",
            "final_mvp_readiness_boundary",
        ]:
            with self.subTest(gate_id=gate_id):
                self.assertIn(gate_id, gate_ids)
        for card in pack["phase2_gate_status_cards"]:
            for field in [
                "gate_status_id",
                "gate_label",
                "gate_group",
                ...
                with self.subTest(gate=card["gate_status_id"], field=field):
                    self.assertIn(field, card)
            self.assertFalse(card["real_execution_allowed"])

        capability_ids = {
            card["unlock_readiness_id"].replace("unlock_readiness_", "")
            for card in pack["real_capability_unlock_readiness_cards"]
        }
        for capability_id in [
            "real_database_persistence",
            "real_llm_generation",
            "real_provider_call",
            "real_file_export",
            "real_platform_upload",
            "real_task_creation",
            "real_approval_token_issue",
            "real_external_call",
            "real_policy_check",
            "real_audit_logging",
        ]:
        ...
        candidate_text = " ".join(
            " ".join(str(value) for value in card.values())
            for card in pack["minimum_real_integration_candidate_cards"]
        )
        self.assertIn("Database persistence minimal adapter candidate", candidate_text)
        self.assertIn("not ready", candidate_text)
        self.assertIn("LLM, provider, upload, and task capabilities remain blocked", candidate_text)
        for card in pack["minimum_real_integration_candidate_cards"]:
            self.assertFalse(card["ready_for_real_integration"])
            self.assertFalse(card["real_execution_allowed"])

        validation_text = " ".join(
            " ".join(str(value) for value in card.values())
            for card in pack["phase2_validation_matrix_cards"]
        )
        for expected in [
            "unit tests",
            "contract tests",
        ...
        dependency_text = " ".join(
            " ".join(str(value) for value in card.values())
            for card in pack["cross_gate_dependency_cards"]
        )
        for expected in [
            "DB gate -> mock harness",
            "LLM gate -> provider unlock review",
            "secret/network -> provider call",
            "approval token -> real execution",
            "audit sink -> production readiness",
            "rollback plan -> unlock decision",
        ]:
            with self.subTest(dependency=expected):
                self.assertIn(expected, dependency_text)
        for card in pack["cross_gate_dependency_cards"]:
            self.assertFalse(card["real_execution_allowed"])

        for card in pack["operator_decision_review_cards"]:
            self.assertFalse(card["real_task_created"])
            self.assertFalse(card["real_approval_created"])
            self.assertFalse(card["real_execution_allowed"])

        gaps_text = " ".join(card["gap_label"] for card in pack["production_readiness_gap_cards"])
        for gap in [
            "no real DB config",
            "no schema migration",
            "no audit sink",
            ...
            with self.subTest(gap=gap):
                self.assertIn(gap, gaps_text)

        risk_text = " ".join(card["risk_label"] for card in pack["phase2_risk_register_cards"])
        for risk in [
            "data persistence risk",
            "secret exposure risk",
            "external call risk",
            "provider cost risk",
            "unsafe claim output risk",
            "audit gap risk",
            "rollback gap risk",
            "policy legal uncertainty",
            "platform upload risk",
        ]:
            with self.subTest(risk=risk):
                self.assertIn(risk, risk_text)

        for card in pack["phase2_next_step_recommendation_cards"]:
            self.assertFalse(card["direct_real_unlock_recommended"])
            self.assertFalse(card["real_execution_allowed"])
        next_step_text = " ".join(
            card["recommendation_label"]
            for card in pack["phase2_next_step_recommendation_cards"]
        )
        self.assertIn("Phase 2 Closeout Review", next_step_text)
        self.assertIn("Real DB Minimal Adapter Contract Preview", next_step_text)

        for card in pack["phase2_final_blocker_cards"]:
            self.assertFalse(card["ready_for_real_execution"])
            self.assertTrue(card["blocks_real_unlock"])
            self.assertFalse(card["real_execution_allowed"])

        checks = pack["phase2_readiness_quality_checks"]
        for key in [
            "gate_status_covered",
            "unlock_readiness_covered",
            "minimum_candidate_covered",
            "validation_matrix_covered",
            "cross_gate_dependencies_covered",
            "operator_decisions_covered",
            "production_gaps_covered",
            "risk_register_covered",
            "next_step_recommendations_covered",
            "final_blockers_covered",
            "safety_boundary_covered",
        ]:
            with self.subTest(check=key):
                self.assertTrue(checks[key])
        for key in [
            "real_database_persistence_performed",
            "real_llm_generation_performed",
            "real_provider_call_performed",
            "secret_read_performed",
            "external_call_performed",
            "token_issue_performed",
    ...
    def test_workspace_phase2_real_db_minimal_adapter_contract_pack_is_preview_only(self):
        payload = {
            "workspace_id": "workspace-phase2-real-db-adapter-contract-preview",
            "source": "manual_import",
            "output_language": "en",
            "products": [{
                "platform": "manual",
                "asin": "REALDBPREVIEW001",
                "title": "Adjustable Laptop Stand",
                "reviews": [
                    {
                        "rating": 2,
        ...
        self.assertEqual(
            pack["pack_version"],
            "workspace_phase2_real_db_minimal_adapter_contract_pack_v1",
        )
        for required_key in [
        ...
        summary = pack["real_db_adapter_contract_summary"]
        self.assertIn("phase2_real_db_minimal_adapter_contract_preview", summary["mode"])
        self.assertIn("deterministic_db_adapter_contract", summary["mode"])
        self.assertIn("dry_run_only", summary["mode"])
        for source_pack in [
            "workspace_phase2_readiness_review_pack",
            "workspace_phase2_persistence_mock_harness_pack",
        ...
        interface_ids = {
            card["db_adapter_contract_id"]
            for card in pack["db_adapter_interface_contract_cards"]
        }
        for expected in [
            ...
            with self.subTest(interface=expected):
                self.assertIn(expected, interface_ids)
        for card in pack["db_adapter_interface_contract_cards"]:
            for field in [
                "db_adapter_contract_id",
                "adapter_label",
                "adapter_group",
                "source_gate_refs",
                "intended_operation",
                ...
                "transaction_boundary_note",
                "risk_note",
            ]:
                with self.subTest(interface=card["db_adapter_contract_id"], field=field):
                    self.assertIn(field, card)
        ...
        schema_tables = {
            ...
            for card in pack["db_schema_mapping_preview_cards"]
        }
        for table in [
            "workspace_sessions",
        ...
        for card in pack["db_schema_mapping_preview_cards"]:
            self.assertTrue(card["schema_migration_required"])
            self.assertFalse(card["real_database_write_allowed"])
            self.assertIn("provider_secret", card["excluded_fields"])

        boundary_text = " ".join(
            " ".join(str(value) for value in card.values())
            for card in pack["db_operation_boundary_cards"]
        )
        for forbidden in [
            "create table",
            "migrate",
            ...
            with self.subTest(forbidden=forbidden):
                self.assertIn(forbidden, boundary_text)
        for card in pack["db_operation_boundary_cards"]:
            self.assertFalse(card["real_database_connection_allowed"])
            self.assertFalse(card["real_database_write_allowed"])
            self.assertFalse(card["real_execution_allowed"])
        ...
        mock_text = " ".join(
            ...
            for card in pack["db_mock_invocation_contract_cards"]
        )
        for operation in ["save", "read", "list", "validation"]:
            self.assertIn(operation, mock_text)
        for card in pack["db_mock_invocation_contract_cards"]:
            self.assertFalse(card["writes_real_database"])
            self.assertFalse(card["reads_real_database"])
            self.assertFalse(card["writes_real_file"])
        ...
        error_text = " ".join(
            card["error_label"] for card in pack["db_error_taxonomy_cards"]
        )
        for error in [
            "connection missing",
            "schema missing",
        ...
        for card in pack["db_error_taxonomy_cards"]:
            self.assertFalse(card["requires_real_rollback"])
            self.assertFalse(card["real_database_write_allowed"])
            self.assertFalse(card["real_execution_allowed"])

        for card in pack["db_audit_trace_contract_cards"]:
            self.assertIn("future_audit_event_shape", card)
            self.assertIn("trace_ids_required", card)
            self.assertIn("actor_refs_required", card)
            self.assertIn("operation_refs_required", card)
            self.assertFalse(card["database_write_allowed"])
            self.assertFalse(card["real_audit_event_created"])
            self.assertFalse(card["real_execution_allowed"])
        ...
        permission_ids = {
            ...
            for card in pack["db_permission_boundary_cards"]
        }
        ...
        redaction_text = " ".join(
            " ".join(str(value) for value in card.values())
            for card in pack["db_redaction_retention_contract_cards"]
        )
        for label in [
            "provider secret",
            "customer data",
            ...
            self.assertIn(label, redaction_text)
        provider_secret_cards = [
            card for card in pack["db_redaction_retention_contract_cards"]
            if card["data_label"] == "provider secret"
        ]
        self.assertTrue(provider_secret_cards)
        for card in provider_secret_cards:
            self.assertFalse(card["db_persistence_allowed"])
            self.assertFalse(card["secret_read_allowed"])
            self.assertFalse(card["real_database_write_allowed"])

        migration_text = " ".join(
            card["dependency_label"]
            for card in pack["db_migration_dependency_cards"]
        )
        for dependency in [
            "schema migration",
            "rollback plan",
            ...
            with self.subTest(dependency=dependency):
                self.assertIn(dependency, migration_text)
        for card in pack["db_migration_dependency_cards"]:
            self.assertFalse(card["implemented"])
            self.assertTrue(card["blocks_real_db_unlock"])

        test_plan_text = " ".join(
            card["test_label"] for card in pack["db_adapter_test_plan_cards"]
        )
        for test_label in [
            "unit tests",
            "contract tests",
        ...
        blocker_text = " ".join(
            card["blocker_label"]
            for card in pack["phase2_real_db_adapter_unlock_blockers"]
        )
        for blocker in [
        ...
        checks = pack["real_db_adapter_quality_checks"]
        for key in [
            "adapter_interface_covered",
            "schema_mapping_covered",
            "operation_boundary_covered",
            "mock_invocation_covered",
            "error_taxonomy_covered",
            "audit_trace_covered",
            "permission_boundary_covered",
            "redaction_retention_covered",
            "migration_dependency_covered",
            "test_plan_covered",
            "unlock_blockers_covered",
            "safety_boundary_covered",
        ]:
    ...
    def test_workspace_phase2_llm_sandbox_contract_pack_is_preview_only(self):
        payload = {
            "workspace_id": "workspace-phase2-llm-sandbox-contract",
            "source": "manual_import",
            "output_language": "en",
            "products": [{
                "platform": "manual",
                "asin": "LLMSANDBOXCONTRACT001",
                "title": "Adjustable Laptop Stand",
                "reviews": [
                    {
                        "rating": 2,
        ...
        self.assertEqual(response.status_code, 200)
        creative_pack = response.json()["creative_decision_pack"]
        self.assertIn("workspace_phase2_llm_sandbox_contract_pack", creative_pack)
        for existing_pack in [
        ...
        pack = creative_pack["workspace_phase2_llm_sandbox_contract_pack"]
        self.assertEqual(
            pack["pack_version"],
            "workspace_phase2_llm_sandbox_contract_pack_v1",
        )
        for required_key in [
        ...
        summary = pack["llm_sandbox_contract_summary"]
        self.assertIn("phase2_llm_sandbox_contract_preview", summary["mode"])
        self.assertIn("deterministic_llm_sandbox_contract", summary["mode"])
        self.assertIn("dry_run_only", summary["mode"])
        for source_pack in [
            "workspace_phase2_audit_sink_contract_pack",
            "workspace_phase2_llm_provider_gate_pack",
        ...
        provider_ids = {
            card["sandbox_provider_profile_id"]
            for card in pack["llm_sandbox_provider_profile_cards"]
        }
        for profile_id in [
            "text_generation_sandbox",
            "claim_safe_copy_sandbox",
            "delivery_qa_assist_sandbox",
            "remediation_suggestion_sandbox",
            "dossier_summary_sandbox",
            "translation_sandbox",
            "policy_reasoning_sandbox",
            "prompt_redaction_sandbox",
        ]:
            with self.subTest(profile_id=profile_id):
                self.assertIn(profile_id, provider_ids)
        for card in pack["llm_sandbox_provider_profile_cards"]:
            for field in [
                "sandbox_provider_profile_id",
                "provider_label",
                "provider_group",
        ...
        fixture_ids = {
            card["prompt_fixture_id"]
            for card in pack["llm_sandbox_prompt_fixture_cards"]
        }
        for fixture_id in [
            "claim_safe_brief_fixture",
            "creative_output_fixture",
            "platform_delivery_fixture",
            "delivery_qa_fixture",
            "remediation_fixture",
            "final_export_summary_fixture",
            "campaign_dossier_summary_fixture",
            "demo_walkthrough_fixture",
        ]:
        ...
        for card in pack["llm_sandbox_prompt_fixture_cards"]:
            ...
            self.assertFalse(card["real_llm_generation_allowed"])

        for card in pack["llm_sandbox_invocation_contract_cards"]:
            self.assertIn("simulated_request_shape", card)
            self.assertIn("simulated_response_shape", card)
            self.assertFalse(card["sandbox_call_executed"])
            self.assertFalse(card["real_provider_call_allowed"])
            self.assertFalse(card["external_call_allowed"])
            self.assertFalse(card["secret_read_allowed"])
            self.assertFalse(card["paid_operation_allowed"])
        ...
        response_schema_text = " ".join(
            " ".join(card["required_evidence_trace_fields"])
            + " "
            ...
            for card in pack["llm_sandbox_response_schema_cards"]
        )
        for expected in [
            "supporting_quote_ids",
            "claim_id",
        ...
        guard_text = " ".join(
            card["sandbox_claim_guard_id"]
            + " "
            + card["guard_label"].lower()
            + " "
            + card["unsupported_claim_behavior"]
            + " "
            + card["missing_evidence_behavior"]
            + " "
            + " ".join(card["do_not_claim_refs"])
            + " "
            + " ".join(card["restricted_claim_refs"])
            for card in pack["llm_sandbox_evidence_claim_guard_cards"]
        )
        for guard in [
            "unsupported claim",
            "missing evidence",
        ...
        privacy_labels = {
            card["data_label"]
            for card in pack["llm_sandbox_redaction_privacy_cards"]
        }
        for label in [
            "provider secret",
            "customer data",
            ...
            with self.subTest(label=label):
                self.assertIn(label, privacy_labels)
        provider_secret = next(
            card for card in pack["llm_sandbox_redaction_privacy_cards"]
            if card["data_label"] == "provider secret"
        )
        self.assertFalse(provider_secret["allowed_in_prompt_input"])
        self.assertFalse(provider_secret["secret_read_allowed"])
        self.assertFalse(provider_secret["audit_persistence_allowed"])

        for card in pack["llm_sandbox_cost_quota_cards"]:
            self.assertIn("sandbox_max_tokens", card)
            self.assertIn("timeout_policy", card)
            self.assertIn("quota_policy", card)
        ...
        audit_trace_text = " ".join(
            ...
            for card in pack["llm_sandbox_audit_trace_cards"]
        )
        for field in [
            "trace id",
            "run id",
        ...
        failure_text = " ".join(
            card["simulated_failure_type"]
            for card in pack["llm_sandbox_failure_recovery_cards"]
        )
        for failure in [
        ...
        test_plan_text = " ".join(
            card["test_label"] for card in pack["llm_sandbox_test_plan_cards"]
        )
        for test_label in [
            "unit tests",
            "contract tests",
        ...
        blocker_text = " ".join(
            card["blocker_label"]
            for card in pack["phase2_llm_sandbox_unlock_blockers"]
        )
        for blocker in [
        ...
        checks = pack["llm_sandbox_quality_checks"]
        for key in [
            "sandbox_provider_profile_covered",
            "prompt_fixtures_covered",
            "invocation_contract_covered",
            "response_schema_covered",
            "evidence_claim_guard_covered",
            "redaction_privacy_covered",
            "cost_quota_covered",
            "audit_trace_covered",
            "failure_recovery_covered",
        ...
        for key in [
            ...
            "media_enabled", "external_scraping",
            "external_scraping_enabled", "database_persistence",
            "database_persistence_enabled", "database_read",
            "database_read_enabled", "database_write",
            "database_write_enabled", "schema_migration",
    ...
    def test_workspace_phase2_sandbox_contract_test_matrix_pack_is_preview_only(self):
        payload = {
            "workspace_id": "workspace-phase2-sandbox-contract-test-matrix",
            "source": "manual_import",
            "output_language": "en",
            "products": [{
                "platform": "manual",
                "asin": "SANDBOXTESTMATRIX001",
                "title": "Adjustable Laptop Stand",
                "reviews": [
                    {
                        "rating": 2,
        ...
        self.assertEqual(
            pack["pack_version"],
            "workspace_phase2_sandbox_contract_test_matrix_pack_v1",
        )
        for required_key in [
            "sandbox_contract_test_matrix_summary",
            "sandbox_provider_test_coverage_cards",
            "sandbox_fixture_validation_cards",
            "sandbox_request_response_test_cards",
            "sandbox_asset_boundary_test_cards",
            "sandbox_policy_claim_guard_test_cards",
            "sandbox_cost_quota_test_cards",
            "sandbox_audit_trace_test_cards",
            "sandbox_failure_recovery_test_cards",
            "sandbox_permission_boundary_test_cards",
            "sandbox_unlock_simulation_cards",
            "phase2_sandbox_test_matrix_blockers",
            "sandbox_test_matrix_quality_checks",
            "audit_preview",
            "safety_boundaries",
        ]:
            with self.subTest(required_key=required_key):
        ...
        summary = pack["sandbox_contract_test_matrix_summary"]
        self.assertIn("phase2_sandbox_contract_test_matrix_preview", summary["mode"])
        self.assertIn("deterministic_unlock_simulation", summary["mode"])
        self.assertIn("dry_run_only", summary["mode"])
        self.assertTrue(summary["dry_run_only"])
        for source_pack in [
            "workspace_phase2_provider_sandbox_contract_pack",
        ...
        coverage_ids = {
            card["sandbox_test_coverage_id"]
            for card in pack["sandbox_provider_test_coverage_cards"]
        }
        for coverage_id in [
            "llm_text_generation_sandbox",
            "image_generation_sandbox",
            "video_generation_sandbox",
            "media_storage_sandbox",
            "analytics_tracking_sandbox",
            ...
            with self.subTest(coverage_id=coverage_id):
                self.assertIn(coverage_id, coverage_ids)
        for card in pack["sandbox_provider_test_coverage_cards"]:
            for field in [
                "sandbox_test_coverage_id",
                "coverage_label",
                "coverage_group",
                "sandbox_capability_type",
                ...
                self.assertIn(field, card)
            self.assertFalse(card["real_sandbox_test_executed"])
            self.assertFalse(card["real_provider_client_created"])
            self.assertFalse(card["real_provider_call_allowed"])
            self.assertFalse(card["secret_read_allowed"])
            self.assertFalse(card["external_call_allowed"])
        ...
        fixture_text = " ".join(
            " ".join(card["forbidden_fixture_inputs"])
            for card in pack["sandbox_fixture_validation_cards"]
        )
        for forbidden in [
            "provider_secret",
            "real_customer_data",
            ...
            self.assertIn(forbidden, fixture_text)
        for card in pack["sandbox_fixture_validation_cards"]:
            self.assertFalse(card["contains_provider_secret"])
            self.assertFalse(card["contains_real_customer_data"])
            self.assertFalse(card["contains_media_binary"])
            self.assertFalse(card["real_call_input_allowed"])

        request_response_text = " ".join(
            card["test_label"]
            + " "
            + " ".join(card["required_fields_or_behavior"])
            for card in pack["sandbox_request_response_test_cards"]
        )
        for expected in [
            "request contract",
            "response schema",
            ...
            self.assertIn(expected, request_response_text)

        asset_boundary_text = " ".join(
            card["asset_boundary_label"]
            for card in pack["sandbox_asset_boundary_test_cards"]
        )
        for asset_label in [
            "image asset",
            "video asset",
        ...
        policy_text = " ".join(
            card["claim_guard_label"]
            for card in pack["sandbox_policy_claim_guard_test_cards"]
        )
        for label in [
            "unsupported claim",
            "missing evidence",
            ...
            self.assertIn(label, policy_text)
        for card in pack["sandbox_policy_claim_guard_test_cards"]:
            self.assertFalse(card["real_policy_check_allowed"])
            self.assertFalse(card["legal_advice_provided"])
            self.assertFalse(card["platform_compliance_conclusion_provided"])

        cost_text = " ".join(
            card["cost_quota_label"]
            for card in pack["sandbox_cost_quota_test_cards"]
        )
        for label in [
            "quota",
            "timeout",
            ...
            "rate limit",
            "retry cap",
            "paid operation disabled",
        ]:
            self.assertIn(label, cost_text)
        for card in pack["sandbox_cost_quota_test_cards"]:
            self.assertFalse(card["billing_request_sent"])
            self.assertFalse(card["real_request_sent"])
            self.assertFalse(card["paid_operation_allowed"])
        ...
        audit_text = " ".join(
            card["audit_trace_field"]
            for card in pack["sandbox_audit_trace_test_cards"]
        )
        for field in [
            "trace id",
            "run id",
            ...
            self.assertIn(field, audit_text)
        for card in pack["sandbox_audit_trace_test_cards"]:
            self.assertFalse(card["database_write_allowed"])
            self.assertFalse(card["audit_event_created"])
            self.assertFalse(card["real_log_read_performed"])

        failure_text = " ".join(
            card["failure_label"]
            for card in pack["sandbox_failure_recovery_test_cards"]
        )
        for failure in [
            "provider unavailable",
            "timeout",
        ...
        permissions = {
            card["permission"]
            for card in pack["sandbox_permission_boundary_test_cards"]
        }
        for permission in [
            "provider",
        ...
        for card in pack["sandbox_permission_boundary_test_cards"]:
            self.assertEqual(card["permission_status"], "disabled")
            self.assertFalse(card["enabled"])
            self.assertFalse(card["real_execution_allowed"])

        for card in pack["sandbox_unlock_simulation_cards"]:
            self.assertEqual(card["simulation_status"], "blocked")
            self.assertEqual(card["unlock_readiness"], "not_ready")
            self.assertEqual(card["unlock_mode"], "preview_only")
            self.assertFalse(card["ready_for_real_sandbox"])
            self.assertFalse(card["ready_for_real_provider"])

        blocker_text = " ".join(
            card["blocker_label"]
            for card in pack["phase2_sandbox_test_matrix_blockers"]
        )
        for blocker in [
            "no provider sandbox key approval",
            "no secret access approval",
        ...
        checks = pack["sandbox_test_matrix_quality_checks"]
        for key in [
            "provider_test_coverage_covered",
            "fixture_validation_covered",
            "request_response_tests_covered",
            "asset_boundary_tests_covered",
            "policy_claim_guard_tests_covered",
            "cost_quota_tests_covered",
            "audit_trace_tests_covered",
            "failure_recovery_tests_covered",
            "permission_boundary_tests_covered",
            "unlock_simulation_covered",
            "blockers_covered",
            "safety_boundary_covered",
        ]:
            self.assertTrue(checks[key])
        for key in [
            "real_sandbox_test_performed",
            "real_provider_call_performed",
            "real_llm_generation_performed",
            "secret_read_performed",
            "external_call_performed",

...
class AgentRunnerFinalizationTests(unittest.TestCase):
    ...
    def test_provider_registry_operation_approval_report_models_approval_simulation_and_persistence_safety(self):
        from agent_runs import (
            build_agent_contract_completeness_report,
            build_agent_contract_registry,
            build_keyframe_prompt_pack_report,
        ...
        project_id = "project_provider_registry_operation_approval"
        agent_report = build_agent_contract_completeness_report(build_agent_contract_registry())
        source_report = build_source_adapter_contract_report()
        output_report = build_multi_agent_output_chain_report(
            agent_contract_report=agent_report,
        ...
        report = build_provider_registry_operation_approval_report(
        ...
        self.assertEqual(
            report["provider_registry_operation_approval_report_version"],
        ...
        self.assertTrue(report["provider_artifact_registry_restore_ready"])
        self.assertTrue(report["operator_review_required"])
        self.assertGreater(report["stage_count"], 0)
        for section in [
    ...
    def test_provider_transaction_monitor_report_models_drift_abort_timeline_and_safety(self):
        from agent_runs import (
            build_agent_contract_completeness_report,
            build_agent_contract_registry,
            build_keyframe_prompt_pack_report,
        ...
        project_id = "project_provider_transaction_monitor"
        agent_report = build_agent_contract_completeness_report(build_agent_contract_registry())
        source_report = build_source_adapter_contract_report()
        output_report = build_multi_agent_output_chain_report(
            agent_contract_report=agent_report,
        ...
        transaction_rehearsal_report = build_provider_registry_transaction_rehearsal_report(
            provider_registry_operation_approval_report=operation_approval_report,
            project_id=project_id,
            requested_by="unit_test",
        )
        report = build_provider_transaction_monitor_report(
            provider_registry_transaction_rehearsal_report=transaction_rehearsal_report,
            project_id=project_id,
            requested_by="unit_test",
        )

        self.assertEqual(
            report["provider_transaction_monitor_report_version"],
        ...
        self.assertTrue(report["provider_registry_transaction_rehearsal_ready"])
        self.assertTrue(report["operator_review_required"])
        self.assertGreater(report["stage_count"], 0)
        for section in [
        ...
        for blocking_failure in [
            ...
            self.assertIn(blocking_failure, report["blocking_failures"])
        self.assertTrue(report["dry_run"])
        self.assertTrue(report["monitoring_required"])
        for disabled_field in [
            "monitoring_started",
    ...
    def test_provider_transaction_incident_drill_report_models_recovery_replay_and_safety(self):
        from agent_runs import (
            build_agent_contract_completeness_report,
            build_agent_contract_registry,
            build_keyframe_prompt_pack_report,
        ...
        project_id = "project_provider_transaction_incident_drill"
        agent_report = build_agent_contract_completeness_report(build_agent_contract_registry())
        source_report = build_source_adapter_contract_report()
        output_report = build_multi_agent_output_chain_report(
            agent_contract_report=agent_report,
        ...
        queue_report = build_provider_queue_lease_worker_report(
            provider_observability_report=observability_report,
            project_id=project_id,
            requested_by="unit_test",
        )
        checkpoint_report = build_provider_worker_checkpoint_resume_report(
            provider_queue_lease_worker_report=queue_report,
            project_id=project_id,
            requested_by="unit_test",
        )
        finalization_report = build_provider_worker_finalization_report(
        ...
        approval_report = build_provider_registry_operation_approval_report(
            provider_artifact_registry_restore_report=registry_restore_report,
            project_id=project_id,
            requested_by="unit_test",
        )
        rehearsal_report = build_provider_registry_transaction_rehearsal_report(
            provider_registry_operation_approval_report=approval_report,
            project_id=project_id,
            requested_by="unit_test",
        )
        monitor_report = build_provider_transaction_monitor_report(
            provider_registry_transaction_rehearsal_report=rehearsal_report,
            project_id=project_id,
            requested_by="unit_test",
        )
        report = build_provider_transaction_incident_drill_report(
            provider_transaction_monitor_report=monitor_report,
            project_id=project_id,
            requested_by="unit_test",
        )

        self.assertEqual(
            report["provider_transaction_incident_drill_report_version"],
        ...
        self.assertTrue(report["provider_transaction_monitor_ready"])
        self.assertTrue(report["operator_review_required"])
        for section in [
        ...
        self.assertTrue(report["dry_run"])
        self.assertTrue(report["incident_drill_required"])
        for disabled_field in [
    ...
    def test_provider_queue_lease_worker_report_models_queue_claim_lease_and_worker_safety(self):
        ...
        output_report = build_multi_agent_output_chain_report(
            agent_contract_report=agent_report,
            source_adapter_contract_report=source_report,
            project_id="project_provider_queue_lease_worker",
            requested_by="unit_test",
        )
        asset_report = build_keyframe_video_asset_chain_report(
            multi_agent_output_chain_report=output_report,
        ...
        report = build_provider_queue_lease_worker_report(
            provider_observability_report=observability_report,
            project_id="project_provider_queue_lease_worker",
            requested_by="unit_test",
        )

        self.assertEqual(report["provider_queue_lease_worker_report_version"], "provider_queue_lease_worker_report_v1")
        self.assertEqual(report["report_status"], "provider_queue_lease_worker_ready_dry_run")
        self.assertTrue(report["provider_observability_ready"])
        self.assertTrue(report["operator_review_required"])
        self.assertGreater(report["stage_count"], 0)
        self.assertTrue(report["queue_model"])
        self.assertTrue(report["idempotency_dedupe_policy"])
        self.assertTrue(report["claim_policy"])
        self.assertTrue(report["lease_policy"])
        self.assertTrue(report["heartbeat_policy"])
        self.assertTrue(report["stale_lease_recovery"])
        self.assertTrue(report["worker_invocation_envelope"])
        self.assertTrue(report["completion_ack"])
        self.assertTrue(report["audit_receipt"])
        self.assertGreater(report["queue_item_schema_count"], 0)
        self.assertGreater(report["recovery_step_count"], 0)
        self.assertGreater(report["required_gate_reference_count"], 0)
        self.assertGreater(report["missing_gate_reference_count"], 0)
        self.assertTrue(report["supports_provider_queue"])
        self.assertTrue(report["supports_idempotency_dedupe"])
        self.assertTrue(report["supports_queue_claim"])
        self.assertTrue(report["supports_worker_lease"])
        self.assertTrue(report["supports_worker_heartbeat"])
        self.assertTrue(report["supports_stale_lease_recovery"])
        self.assertTrue(report["supports_worker_invocation_envelope"])
        self.assertTrue(report["supports_completion_ack"])
        self.assertTrue(report["supports_queue_worker_audit_receipt"])
        self.assertTrue(report["dry_run"])
        self.assertFalse(report["queue_insert_allowed"])
        self.assertFalse(report["queue_item_created"])
        self.assertFalse(report["queue_persisted"])
        self.assertFalse(report["duplicate_job_submitted"])
        self.assertFalse(report["dedupe_record_persisted"])
        self.assertFalse(report["claim_allowed"])
        self.assertFalse(report["claim_persisted"])
        self.assertFalse(report["lease_allowed"])
        self.assertFalse(report["lease_acquired"])
        self.assertFalse(report["lease_persisted"])
        self.assertFalse(report["heartbeat_recorded"])
        self.assertFalse(report["worker_alive"])
        self.assertFalse(report["worker_started"])
        self.assertFalse(report["worker_loop_started"])
        self.assertFalse(report["worker_invocation_allowed"])
        self.assertFalse(report["worker_invocation_performed"])
        self.assertFalse(report["completion_recorded"])
        self.assertFalse(report["queue_item_completed"])
        self.assertFalse(report["lease_released"])
        self.assertFalse(report["queue_item_requeued"])
        self.assertFalse(report["real_execution_allowed"])
        self.assertFalse(report["real_execution_enabled"])
        self.assertFalse(report["provider_call_allowed"])
        self.assertFalse(report["external_api_call_allowed"])
